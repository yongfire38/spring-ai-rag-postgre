...
public class EgovEnhancedDocumentTransformer implements DocumentTransformer {
    ...
    // 요약 생성 여부를 제어하는 설정
    @Value("${spring.ai.document.enable-summary:false}")
    private boolean enableSummary;

    // 요약을 생성할 최소 청크 수
    @Value("${spring.ai.document.summary-min-chunks:5}")
    private int summaryMinChunks;

    // 청크 크기 설정
    @Value("${spring.ai.document.chunk-size:4000}")
    ...
    // 최대 청크 수
    @Value("${spring.ai.document.max-num-chunks:500}")
    private int maxNumChunks;

    // 임베딩할 최소 청크 길이
    @Value("${spring.ai.document.min-chunk-length-to-embed:50}")
    private int minChunkLengthToEmbed;

    // 키워드 추출 활성화 여부
    @Value("${spring.ai.document.enable-keywords:false}")
    private boolean enableKeywords;

    // 추출할 키워드 개수
    @Value("${spring.ai.document.keyword-count:5}")
